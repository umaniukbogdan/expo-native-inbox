...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    // Подписываемся на события обновления истории уведомлений (реактивное обновление)
    console.log("📡 Subscribing to notification history updates...");
    const unsubscribe = subscribeToNotificationHistory(
      async (newNotificationData, totalCount) => {
        console.log("🔄 New notification received, updating history...", {
          newNotificationData,
          totalCount,
        });
        // Перезагружаем всю историю чтобы получить актуальный список
    ...
    return () => {
      ...
      unsubscribe(); // Отписываемся от событий истории уведомлений
